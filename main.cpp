...
int main()
{
	...
	lucesFeria[0] = PointLight(1.0f, 0.8f, 0.6f,
		0.6f, 0.4f,
		-150.0f, 50.0f, 150.0f,   // Esquina superior izquierda
		0.02f, 0.002f, 0.0003f);
	lucesFeriaCount++;

	lucesFeria[1] = PointLight(1.0f, 0.8f, 0.6f,
		0.6f, 0.4f,
		150.0f, 50.0f, 150.0f,    // Esquina superior derecha
		0.02f, 0.002f, 0.0003f);
	lucesFeriaCount++;

	lucesFeria[2] = PointLight(1.0f, 0.8f, 0.6f,
		0.6f, 0.4f,
		-150.0f, 50.0f, -150.0f,  // Esquina inferior izquierda
		0.02f, 0.002f, 0.0003f);
	lucesFeriaCount++;

	lucesFeria[3] = PointLight(1.0f, 0.8f, 0.6f,
		0.6f, 0.4f,
		140.0f, 50.0f, -150.0f,   // Esquina inferior derecha
		0.02f, 0.002f, 0.0003f);
	lucesFeriaCount++;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (juegoactivo == 3 && monedamovy < -7.0f) {
			...
			if (dardomovX < -43.0f) {
				dardomovX -= 1.0f * deltaTime * 0.0f;
				dardomovZ += 0.5f * deltaTime * 0.0f;
				dardogiroY += 1.0f * deltaTime * 0.0f;
				globoscale = 0.0f;
			}

		}
		else { dardomovX = 0.0f, dardomovZ = 0.0f, dardogiroY = 0.0f, globoscale = 1.0f; }
